# Generated from Group.g4 by ANTLR 4.9.2
from antlr4 import *
if __name__ is not None and "." in __name__:
    from .GroupParser import GroupParser
else:
    from GroupParser import GroupParser

# This class defines a complete listener for a parse tree produced by GroupParser.
class GroupListener(ParseTreeListener):

    # Enter a parse tree produced by GroupParser#hi.
    def enterHi(self, ctx:GroupParser.HiContext):
        pass

    # Exit a parse tree produced by GroupParser#hi.
    def exitHi(self, ctx:GroupParser.HiContext):
        pass



del GroupParser